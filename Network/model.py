...
EMB_SUBDIR = {
    ...
    "structure": "/SAN/bioinf/PFP/embeddings/cafa5_small/graph",
...
class InterlabelGODataset(Dataset):
    def __init__(self,
        ...
        if not self.low_memory:
            if self.embedding_type == "mmsite":
                ...
                for name in tqdm(self.names, desc="Loading structure embeddings"):
    ...
    def load_featureStructure(self, name: str) -> np.ndarray:
        """
        Load structure embedding from npy file.
        ...
        # If the environment variable "PFP_RANDOM_STRUCTURE" is set to a truthy
        # value ("1", "true", "yes"), skip reading the on‑disk embedding and
        # instead return a reproducible random 1 024‑D vector.  This provides
        # an easy way to benchmark the model against a pure‑chance baseline
        ...
        if str(os.getenv("PFP_RANDOM_STRUCTURE", "0")).lower() in {"1", "true", "yes"}:
            ...
            arr = rng.randn(1024).astype(np.float32)  # adjust dim if needed
            return torch.from_numpy(arr).float()

        subdir = EMB_SUBDIR["structure"]
        arr = np.load(
            os.path.join(self.features_dir, subdir, f"{name}.npy"),
            allow_pickle=True,
        )

        # ───────────── Handle object‑array dict wrappers ─────────────
        if isinstance(arr, np.ndarray) and arr.dtype == object:
            arr = arr.item()                      # unwrap the dict
            if isinstance(arr, dict) and "embedding" in arr:
                arr = arr["embedding"]
            else:
                raise ValueError(
                    f"{name}: expected dict with key 'embedding', got {type(arr)}"
                )
    ...
    def _load_feature_file(self, directory: str, name: str) -> torch.Tensor:
        ...
        if self.embedding_type == "esm":
            ...
            # Original behaviour for structure and mmstie
            if isinstance(arr, np.ndarray) and arr.dtype == object:
