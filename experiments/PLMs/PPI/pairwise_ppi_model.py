# experiments/PLMs/gatefuse/pairwise_ppi_model.py
"""Pairwise fusion with optional PPI modality."""
...
class PairwiseWithPPIFusion(nn.Module):
    """Pairwise adaptive fusion with optional PPI complement."""
    
    def __init__(self, modality_pair, dim_config, ppi_dim=512,
        ...
        # Gate network (2 gates for mod1/mod2, PPI handled separately)
        ...
        # Fusion network - input includes PPI flag
        if use_ppi:
            # mod1 + mod2 + interact + ppi + ppi_flag
    ...
    def compute_adaptive_gates(self, h1, h2):
        """Compute gates for mod1 and mod2."""
    ...
    def forward(self, mod1_features, mod2_features, ppi_features=None, ppi_flag=None):
        ...
        if self.use_ppi and ppi_features is not None:
            ...
            # Mask out PPI for proteins without it (zero * flag = zero)
            # This prevents the model from learning patterns from zero vectors
            ppi_h = ppi_h * ppi_flag
