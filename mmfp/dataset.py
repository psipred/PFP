...
Handles missing modalities gracefully via binary masks.
"""
...
class MultiModalDataset(Dataset):
    ...
    Loads pre-computed embeddings for four modalities:
    - Sequence (ProtT5/ESM)
    - Text (PubMedBERT)
    - Structure (ESM-IF)
    - PPI (graph embeddings)

    Supports per-modality normalization and handles missing data via masks.

    Args:
        data_dir: Path to data directory containing labels and protein IDs
    ...
    def _check_coverage(self) -> None:
        """Check modality coverage and print statistics."""
    ...
    def _load_embedding_raw(self, modality: str, protein_id: str, dim: int):
    ...
    def _load_embedding(self, modality: str, protein_id: str, dim: int, mod_key: str):
    ...
    def _normalize(self, emb: np.ndarray, mod_key: str) -> np.ndarray:
