...
class FmaxMetric:
    ...
    def compute_protein_centric_fm(self, y_pred, y_true, margin=0.01, weight_tensor=None, parent_prop=False):
        ...
        # number of validation sample in this batch
        ...
        for i, threshold in enumerate(cut_off_array):
            ...
            #     [1, 1, 1, 0]   # Protein 3
            # ]
            # torch.sum(solidified_y_pred, dim=1) = [2, 0, 3]
            ...
            # true positive predictoins is 1 torch.Size([2880, 1318])
            intersection = torch.logical_and(solidified_y_pred, y_true)
            # #predicted goterms for each protein sample
        ...
        # threshold_index = (other_record_tensor[:, 2] == 0.5).nonzero(as_tuple=True)[0]

        # metrics_tuple = tuple(other_record_tensor[threshold_index].squeeze().tolist())

        # w_precision = metrics_tuple[0]
        # w_recall = metrics_tuple[1]

        # n = 2 * w_precision * w_recall
        # d = w_precision + w_recall
        # w_f1 = n / (d + 1e-16)
        # print(record_f1_tensor[50])
        # exit(w_f1)
        return best_f1, * other_record_tensor[best_f1_idx, :], record_f1_tensor[50]
...
class  Trainer:
    ...
    def evaluate(self, model=None, val_loader=None, margin=0.001, print_loss=False):
        ...
        with torch.no_grad():
            ...
            # 1 * 2800 (all validation) * 1318 (all go terma)
            # print(len(y_preds[0][1]))
            # print(len(y_trues[0][1]))
            ...
            mean_f1_score, mean_precision, mean_recall, cut_off, x = self.metric.compute_protein_centric_fm(y_preds, y_trues, margin=margin, weight_tensor=self.weight_matrix, parent_prop=self.parent_prop)
    ...
    def predict(self):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                for i, (names, features, features1, features2) in tqdm(enumerate(self.PredictLoader),total=len(self.PredictLoader), ascii=' >=', desc='Predicting'):
                    
                    # exit('here')


                    # data, data2, data3 = data.to(self.device), data2.to(self.device), data3.to(self.device), labels.to(self.device)
                    # outputs = self.model(data, data2, data3)


                    protein_ids.extend(names)
                    features, features1, features2  = features.to(self.device), features1.to(self.device), features2.to(self.device)
                    
                    y_pred = self.model(features, features1, features2)
                    if self.back_prop and self.child_matrix is not None:
                        y_pred = self.parent_prop(y_pred, self.child_matrix)
                    y_pred = y_pred.cpu().numpy()
                    y_preds.append(y_pred)
