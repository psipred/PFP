...
NEW (coverage-conditional single-modality eval):
- For *_only settings (seq_only/text_only/struct_only/ppi_only), we evaluate ONLY on proteins
  that originally HAVE that modality (mask==1). Denominator = covered proteins.
- For pair settings (seq_text/seq_struct/seq_ppi), default remains "system-level": all proteins
  (with missing modalities masked to zeros, no filtering).
"""
...
# -----------------------------
# Coverage-conditional helpers
# -----------------------------
def _get_dataset_mask_vector(dataset, modality: str) -> Optional[np.ndarray]:
    """
    Try to extract a fast mask vector (N,) from dataset without iterating __getitem__.
    If not available, return None.
    """
    key = f"{modality}_mask"

    # Common patterns users implement in datasets
    for attr in [key, f"{key}s", f"{modality}_masks", "masks", "mask_dict", "modality_masks"]:
...
def build_subset_by_modality_coverage(
    dataset,
    modality: str
) -> Tuple[torch.utils.data.Subset, List[int]]:
    """
    Build Subset(dataset, idxs) where idxs are proteins that HAVE modality (mask==1) originally.
    Returns (subset, idxs).

    NOTE: If the dataset does not expose masks as arrays, we fall back to iterating __getitem__
    once to read `<modality>_mask` from each item.
    """
    assert modality in ("seq", "text", "struct", "ppi"), f"Unknown modality: {modality}"
    mask_key = f"{modality}_mask"

    v = _get_dataset_mask_vector(dataset, modality)
    if v is not None and len(v) == len(dataset):
        idxs = np.where(v.astype(np.float32) > 0.5)[0].tolist()
        return torch.utils.data.Subset(dataset, idxs), idxs

    # Fallback: iterate (may touch disk if __getitem__ loads embeddings)
    idxs = []
    for i in range(len(dataset)):
        item = dataset[i]
        m = item[mask_key]
        mval = float(m.item()) if torch.is_tensor(m) else float(m)
        if mval > 0.5:
            idxs.append(i)
...
def make_subset_loader_like(
    base_loader: DataLoader,
    subset_dataset
) -> DataLoader:
    """
    Create a DataLoader for subset with same batch size / workers / pin_memory as base_loader.
    """
    return DataLoader(
        subset_dataset,
        ...
        worker_init_fn=worker_init_fn if getattr(base_loader, "num_workers", 0) else None,
    )


# -----------------------------
# CAFA eval wrapper (with eval-time masking)
# -----------------------------
        ...
        class MultimodalWrapper:
            ...
            def __iter__(self):
                for batch in self.loader:
                    ...
                    # eval-time masking: if not kept => zero embedding + zero mask
...
def run_missing_modality_cafa_suite(
    model,
    ...
    fusion_type: str,
    train_labels,
    conditional_single_modality: bool = True,
) -> Dict[str, Dict]:
    """
    Runs CAFA evaluation under multiple eval-time masking settings.

    Key behavior you requested:
    - For "xxx_only" settings, if conditional_single_modality=True:
        Evaluate only on proteins that HAVE that modality (mask==1).
        Denominator = covered proteins.
    - For pair settings: evaluate on all proteins (system-level).
    """
    protocols = {
        # singles (coverage-conditional if enabled)
        "seq_only": ("seq",),
        "text_only": ("text",),
        "struct_only": ("struct",),
        "ppi_only": ("ppi",),

        # pairs (system-level on all proteins)
        "seq_text": ("seq", "text"),
    ...
    out: Dict[str, Dict] = {}
    base_outdir = Path(output_dir) / "cafa_eval_missing"

    for name, keep in protocols.items():
        print(f"\n--- CAFA missing-modality eval: {name} (keep={keep}) ---")

        loader_to_use = test_loader
        protein_ids_to_use = test_protein_ids
        coverage_info = {"n_total": len(test_dataset), "n_eval": len(test_dataset), "coverage": 1.0}

        is_single = name.endswith("_only")
        if conditional_single_modality and is_single:
            mod = keep[0]
            subset, idxs = build_subset_by_modality_coverage(test_dataset, mod)
            loader_to_use = make_subset_loader_like(test_loader, subset)
            protein_ids_to_use = [test_protein_ids[i] for i in idxs]
            ...
            coverage_info = {
                "n_total": int(n_total),
                "n_eval": int(n_eval),
                "coverage": float(n_eval / max(1, n_total)),
                "filtered_on": f"{mod}_mask==1",
            }
            print(f"  [conditional] {mod}: evaluating on {n_eval}/{n_total} proteins (coverage={coverage_info['coverage']:.3f})")

        metrics = evaluate_with_cafa(
            model=model,
            loader=loader_to_use,
            device=device,
            protein_ids=protein_ids_to_use,
            go_terms=go_terms,
            obo_file=obo_file,
        ...
        out[name] = metrics or {}
        # attach coverage meta so it gets persisted
        out[name]["_coverage"] = coverage_info
...
def train_fusion_model(seq_model, aspect, fusion_type, modality_dropout=0.1, output_base='.',
    ...
    if eval_missing and obo_file.exists():
        missing_cafa = run_missing_modality_cafa_suite(
            ...
            fusion_type=fusion_type,
            train_labels=train_dataset.labels,
            conditional_single_modality=True,  # <- your requested behavior
        )
    ...
    if missing_cafa:
        ...
        for setting, m in missing_cafa.items():
            ...
            for k, v in (m or {}).items():
                ...
                # keep coverage meta
                if k == "_coverage" and isinstance(v, dict):
                    results['cafa_missing'][setting][k] = v

            # ensure coverage is present (it is added in run_missing_modality_cafa_suite)
            if "_coverage" in (m or {}):
                results['cafa_missing'][setting]["_coverage"] = m["_coverage"]
...
def run_ensemble_evaluation(model_paths: List[Path], test_loader, device,
    ...
    for path in model_paths:
        f_type = path.parent.name  # .../fusion_type/best_model.pt
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-base', type=str,
                        default='/home/zijianzhou/project/PFP/experiments/pfp_paper/experiment_1_fusion_compare/alldrop')
    ...
    parser.add_argument('--eval-missing', action='store_true',
                        help='Run extra CAFA eval with eval-time modality masking (no retraining). '
                             'Singles are coverage-conditional (mask==1).')
